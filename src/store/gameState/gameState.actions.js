...
const START_GAME_EP = USE_LOCAL
  ? "http://localhost:9001/game/startGame"
  : "https://nicus-dirty-christmas.herokuapp.com/game/startGame";
const REJOIN_GAME_EP = USE_LOCAL
  ? "http://localhost:9001/game/rejoinGame"
  : "https://nicus-dirty-christmas.herokuapp.com/game/rejoinGame";
const SET_PLAYER_ORDER_EP = USE_LOCAL
  ? "http://localhost:9001/game/setPlayerOrder"
  : "https://nicus-dirty-christmas.herokuapp.com/game/setPlayerOrder";
const GET_GAME_DATA_EP = USE_LOCAL
  ? "http://localhost:9001/game/getGameData"
  : "https://nicus-dirty-christmas.herokuapp.com/game/getGameData";
const SET_PLAYER_TURN_EP = USE_LOCAL
  ? "http://localhost:9001/game/setPlayerTurn"
  : "https://nicus-dirty-christmas.herokuapp.com/game/setPlayerTurn";

export const startGame = () => async (dispatch) => {
  try {
    const res = await axios.post(START_GAME_EP, {
...
export const rejoinGame = (userName, pin) => async (dispatch) => {
  const rejoinRes = await axios.post(REJOIN_GAME_EP, {
    data: { userName, pin },
  });
...
export const setIoSocket = (socket) => async (dispatch) => {
  dispatch({ type: gameTypes.SET_SOCKET, payload: socket });
};

export const setPlayerOrder = (playerArray, gameId) => async (dispatch) => {
  try {
    console.log("gameState.actions gameId: ", gameId); //! REMOVE
    const res = await axios.post(SET_PLAYER_ORDER_EP, {
      data: { playerArray, gameId },
    });
